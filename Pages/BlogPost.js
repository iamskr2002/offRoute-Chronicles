...
export default function BlogPost() {
  ...
  const { data: relatedPosts = [] } = useQuery({
    ...
    queryFn: () => base44.entities.BlogPost.filter({ category: post.category }),
    enabled: !!post?.category,
    select: (data) => data.filter(p => p.id !== postId).slice(0, 3),
  });
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div>
                ...
                <ReactionButtons entityType="blog_post" entityId={postId} />
              </div>
